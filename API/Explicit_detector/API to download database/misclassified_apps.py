...
# This script gets the percentage of misclassified apps (text or image explicit) on a database.
...
c.execute(''' SELECT age,id FROM app_data WHERE mature=1''')

for d in c.fetchall():
	if d[0]==18 or is_explicit(d[1]):
		tp+=1
	else:
		fn+=1

c.execute(''' SELECT age,id FROM app_data WHERE mature=0''')
